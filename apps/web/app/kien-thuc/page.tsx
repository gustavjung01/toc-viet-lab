...
export default function KnowledgePage() {
  ...
  return (
    <div className="bg-black text-cream-card">
      <Header />
      <main className="px-4 py-8 pb-24 sm:px-6 lg:px-8 lg:pb-10">
        <div className="mx-auto max-w-7xl">
          <section className="rounded-[2rem] bg-radial-gold p-5 text-white shadow-soft sm:p-7 lg:p-10">
            <div className="grid gap-6 lg:grid-cols-[1fr_.65fr]">
              <div>
                <p className="text-[11px] font-extrabold uppercase tracking-[0.24em] text-champagne sm:text-xs">Kho tri thức ngành tóc</p>
                <h1 className="mt-4 text-3xl font-black tracking-tight sm:text-4xl md:text-5xl">Tra cứu kỹ thuật tóc chuyên sâu</h1>
                <p className="mt-4 max-w-2xl text-sm font-semibold leading-7 text-white/70 sm:text-base">Chủ đề salon Việt: nền tóc, nhuộm màu, phủ bạc, sửa lỗi màu và phục hồi.</p>
                <div className="mt-6 flex max-w-2xl rounded-full bg-white p-2">
                  <Search size={18} className="ml-4 flex-shrink-0 self-center text-warmgray" />
                  <input
                    className="min-w-0 flex-1 bg-transparent px-3 text-sm text-charcoal outline-none"
                    placeholder="Tìm chủ đề, kỹ thuật..."
                    value={query}
                    onChange={(e) => setQuery(e.target.value)}
                  />
                  {query && (
                    <button onClick={() => setQuery("")} className="mr-2 self-center text-xs text-warmgray hover:text-charcoal">✕</button>
                  )}
                  <button className="rounded-full bg-champagne px-4 py-3 text-sm font-extrabold text-charcoal">Tìm</button>
                </div>
              </div>
              <div className="hidden rounded-3xl border border-champagne/25 bg-black/25 p-4 sm:block">
                <HairVisual className="rounded-2xl" gradient="from-[#15211e] via-[#637a4d] to-[#c9a45c]" label="Chủ đề nổi bật" aspect="aspect-video" />
                <h2 className="mt-4 text-xl font-black">Sửa lỗi màu khói bị xanh rêu</h2>
          ...
          <section className="mt-8">
            <div className="mb-4 flex items-end justify-between gap-4">
              ...
              <FilterSheetClient categories={allCategories} activeCategory={activeCategory} onApply={(cat) => handleCategoryChange(cat)} />
            </div>
              ...
              {allCategories.slice(0, 8).map((cat) => (
                <button
                  ...
                  className={`shrink-0 rounded-full px-3 py-2 text-xs font-black transition sm:px-4 sm:text-sm ${
                    activeCategory === cat
                      ? "bg-gold text-black"
                      : "border border-white/10 bg-white/5 text-white/70 hover:border-gold/50 hover:text-gold"
                  }`}
                >
                  {cat}
                </button>
          ...
          <section className="mt-10">
            <div className="mb-5 flex items-center justify-between gap-4">
              <p className="text-sm font-semibold text-white/65">
                <span className="font-extrabold text-white">{total}</span> chủ đề
                {query && <span> cho &ldquo;{query}&rdquo;</span>}
                {activeCategory !== "Tất cả" && <span> trong <span className="text-gold">{activeCategory}</span></span>}
              </p>
              <button className="rounded-full border border-white/10 px-4 py-2 text-xs font-bold text-white/70">Mới nhất</button>
            </div>
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-white/10 py-20 text-center">
                <p className="text-lg font-extrabold text-white">Không tìm thấy kết quả</p>
                <p className="mt-2 text-sm text-white/60">Thử từ khoá khác hoặc chọn danh mục khác</p>
                <button onClick={() => { setQuery(""); setActiveCategory("Tất cả"); }} className="mt-4 rounded-full bg-gold px-5 py-2 text-sm font-bold text-black">Xoá bộ lọc</button>
              </div>
            ) : (
              <>
                ...
                {hasMore && (
                  <div className="mt-10 text-center">
                    <button onClick={handleLoadMore} disabled={loadingMore} className="inline-flex items-center gap-2 rounded-full bg-gold px-8 py-3 text-sm font-extrabold text-black transition hover:bg-champagne disabled:opacity-60">
