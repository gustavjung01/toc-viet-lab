...
export async function GET(req: NextRequest) {
  ...
  let countSql = "SELECT COUNT(*) as total FROM cases WHERE published = 1";
  const params: string[] = [];
  const countParams: string[] = [];

  if (category && category !== "Tất cả" && category !== "Tất cả loại case") {
    sql += " AND category = ?";
  ...
  try {
    ...
    return NextResponse.json({ cases: data.result[0].results, total, limit, offset });
  } catch (e: any) {
