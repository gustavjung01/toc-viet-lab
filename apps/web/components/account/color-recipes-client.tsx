...
export function ColorRecipesClient() {
  const [items, setItems] = useState<ColorRecipe[]>([]);
          ...
          {loading ? (
            ...
            filtered.map((item) => (
              <div
                ...
                className={`grid cursor-pointer gap-4 rounded-3xl bg-white p-4 shadow-soft transition hover:shadow-md md:grid-cols-[1fr_auto] md:items-center ${selected?.id === item.id ? "ring-2 ring-[#D6A84F]" : ""}`}
              >
                <div>
                  {item.tag && (
                  ...
                  <h2 className="mt-2 text-lg font-black text-charcoal">{item.title}</h2>
                  <p className="mt-1 text-sm text-mutedLight">
                    {[item.base, item.developer, item.ratio].filter(Boolean).join(" · ")}
                  </p>
                </div>
                <button
