      ...
      <section className="mt-8 grid gap-5 lg:grid-cols-3">
        {loading ? (
          ...
          <div className="col-span-full rounded-[2rem] border border-dashed border-black/10 bg-white p-10 text-center shadow-soft">
            ...
            <p className="mt-2 text-sm text-mutedLight">Bấm “Đăng tin” để tạo tin đầu tiên. Không cần khai báo salon.</p>
          </div>
        ) : jobs.map((job) => (
          <article key={job.id} className="rounded-[2rem] bg-white p-6 text-charcoal shadow-soft">
            <div className="flex items-center justify-between gap-3">
              <span className="rounded-full bg-cream px-3 py-1 text-xs font-bold text-mutedLight">{job.status === "published" ? "Đang hoạt động" : job.status}</span>
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          {recruitmentPlanCards.map((plan) => (
            <div key={plan.code} className={`rounded-[2rem] bg-white p-6 text-charcoal shadow-soft ${plan.popular ? "ring-2 ring-[#D6A84F]" : ""}`}>
              ...
              <button className="mt-6 w-full rounded-full bg-charcoal px-5 py-3 text-sm font-extrabold text-champagne">Liên hệ mua gói</button>
            </div>
          ))}
        </div>
      </section>
