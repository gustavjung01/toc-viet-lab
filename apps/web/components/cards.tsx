...
export function SectionHeader({ eyebrow, title, desc, href }: { eyebrow?: string; title: string; desc?: string; href?: string }) {
  return (
    <div className="mb-7 flex flex-col justify-between gap-4 md:flex-row md:items-end">
      <div>{eyebrow && <div className="mb-2 text-xs font-extrabold uppercase tracking-[0.25em] text-goldText">{eyebrow}</div>}<h2 className="text-2xl font-extrabold tracking-tight text-charcoal md:text-4xl">{title}</h2>{desc && <p className="mt-3 max-w-2xl text-sm leading-7 text-mutedLight">{desc}</p>}</div>
...
export function CategoryCard({ title, count, icon: Icon }: { title: string; count: string; icon: any }) {
  return <div className="card-hover rounded-3xl border border-black/5 bg-white p-5 shadow-soft"><div className="mb-5 flex h-12 w-12 items-center justify-center rounded-2xl bg-champagne/15 text-goldText"><Icon size={24} /></div><h3 className="font-extrabold text-charcoal">{title}</h3><p className="mt-1 text-sm text-mutedLight">{count}</p></div>;
}
  ...
  return (
    <Link href={`/kien-thuc/${article.slug}`} className="card-hover block overflow-hidden rounded-3xl border border-black/5 bg-white shadow-soft">
      <HairVisual
        ...
        gradient={imageSrc ? undefined : article.visual}
        label={imageSrc ? undefined : article.category}
      />
      <div className="p-5"><div className="mb-3 flex items-center justify-between gap-3"><span className="rounded-full bg-champagne/15 px-3 py-1 text-xs font-bold text-charcoal">{article.level}</span><span className="inline-flex items-center gap-1 text-xs font-semibold text-mutedLight"><Clock size={14} /> {article.minutes} phút</span></div><h3 className="line-clamp-2 text-lg font-extrabold leading-snug text-charcoal">{article.title}</h3><p className="mt-3 line-clamp-2 text-sm leading-6 text-mutedLight">{article.excerpt}</p><div className="mt-5 flex items-center justify-between"><span className="text-sm font-extrabold text-charcoal">Đọc chi tiết</span><BookmarkButton itemType="article" itemId={article.slug} /></div></div>
    </Link>
  );
}
  ...
  return (
    <div className="card-hover overflow-hidden rounded-3xl border border-black/5 bg-white shadow-soft">
      ...
      <div className="p-5">
        ...
        <p className="mt-3 line-clamp-2 text-sm leading-6 text-mutedLight"><b>Tình trạng:</b> {item.condition}</p>
        <p className="mt-1 line-clamp-2 text-sm leading-6 text-mutedLight"><b>Mục tiêu:</b> {item.goal}</p>
        <div className="mt-5 flex items-center justify-between text-sm">
          <span className="font-bold text-charcoal">{item.salon}</span>
          <div className="flex items-center gap-3">
            <span className="text-mutedLight">{item.time}</span>
