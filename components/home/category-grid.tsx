    ...
    <section className="bg-cream px-4 py-12 sm:px-6 sm:py-16 lg:px-8 lg:py-20">
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex flex-col gap-4 sm:mb-12 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <span className="mb-2 inline-block text-xs font-bold uppercase tracking-[0.2em] text-gold">Danh mục kiến thức</span>
          ...
          {categories.map((category) => {
            ...
            return (
              <Link
                key={category.title}
                href={category.href}
                className="group relative overflow-hidden rounded-[2rem] border border-gold/15 bg-cream-card p-6 transition-all duration-300 hover:border-gold/30 hover:shadow-gold-sm"
              >
                <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-2xl bg-gold/10 text-gold transition-transform duration-300 group-hover:scale-105">
                  <Icon className="h-6 w-6" />
                </div>
                <h3 className="mb-1 text-lg font-bold text-charcoal transition-colors group-hover:text-gold">{category.title}</h3>
                <p className="text-sm text-muted">{category.count}</p>
                <ArrowRight className="absolute bottom-6 right-6 h-5 w-5 text-gold opacity-0 transition-all duration-300 group-hover:opacity-100 group-hover:translate-x-1" />
              </Link>
            );
          })}
        </div>
